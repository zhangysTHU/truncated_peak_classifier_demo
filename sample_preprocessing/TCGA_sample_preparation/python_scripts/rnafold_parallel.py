...
sys.stderr.write('Fasta records number: %d\n' %(len(seqtitle)))
...
flat_seq_list=list(primary_sequence)
...
for i in range(0,len(seqtitle)):
    # write every key and value to file
    w.writerow( [seqtitle[i], pvalue_list[i], rnafold_list[i], flat_dotbracket[i], flat_seq_list[i]] )
