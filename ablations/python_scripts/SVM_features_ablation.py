...
# %%
def SVM_kftest(X, y, output_dir = './5fold_features_ablation'):    
    # 创建五折交叉验证
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(X)):
        ...
        # 获取当前折的AUC分数
        y_pred_proba = SVM_rscv.best_estimator_.predict_proba(X_test)[:, 1]
    ...
    results_file_path = os.path.join(path, "SVM_AUCs.csv")
...
def run_ablation_experiment(balanced_df, start, end, description, output_dir='./5fold_features_ablation'):
    """
    参数：
    ...
    """
    print(f"Running ablation experiment: {description}")
    X = balanced_df.iloc[:, start:end].values
    y = balanced_df.iloc[:, -1].values
    SVM_kftest(X, y, output_dir=output_dir)
